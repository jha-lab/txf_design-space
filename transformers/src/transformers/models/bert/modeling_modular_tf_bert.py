...
class TFBertEmbeddingsModular(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.hidden_size = config.hidden_dim_list[0]
...
class TFBertSelfAttentionModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        super().__init__(**kwargs)

        if config.hidden_dim_list[i] % config.attention_heads_list[layer_id] != 0:
            raise ValueError(
                f"The hidden size ({config.hidden_size}) is not a multiple of the number "
                f"of attention heads ({config.num_attention_heads})"
            )

        self.num_attention_heads = config.num_attention_heads_list[layer_id]
        ...
        self.W = tf.get_variable('W_', dtype=tf.float32, shape=[self.attention_head_size,self.attention_head_size],
    ...
    def call(
        ...
        if self.sim=='sdp':
            ...
            attention_scores = tf.matmul(tf.matmul(query_layer, self.W), key_layer, transpose_b=True)
...
# Add function for FFT implemented in Tensorflow
def fftn(x):
    ...
    """
    out = tf.cast(x, dtype='complex64')
    out = tf.math.real(tf.signal.fft(out))
    return out


class TFBertHeteroAttentionModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        super().__init__(**kwargs)
        ...
        for sim_type in self.sim_types:
            if sim_type == 'wma':
                setattr(self, f'W{wma_count}', tf.compat.v1.get_variable('W_', dtype=tf.float32, shape=[self.attention_head_size, self.attention_head_size],
                    initializer=tf.random_uniform_initializer(minval=-0.1, maxval=0.1)))
                wma_count += 1
            elif sim_type.isnumeric():
                setattr(self, f'key_conv_attn_layer{conv_count}', tf.keras.layers.SeparableConv1D(
                    self.attention_head_size,
                    int(sim_type),
                    padding="same",
                    activation=None,
                    depthwise_initializer=get_initializer(1 / int(sim_type)),
                    pointwise_initializer=get_initializer(config.initializer_range),
                    name=f'key_conv_attn_layer{conv_count}'))
                setattr(self, f'conv_kernel_layer{conv_count}', tf.keras.layers.Dense(
                    int(sim_type),
                    activation=None,
                    name=f'conv_kernel_layer{conv_count}',
                    kernel_initializer=get_initializer(config.initializer_range)))
                setattr(self, f'conv_out_layer{conv_count}', tf.keras.layers.Dense(
                    self.attention_head_size,
                    activation=None,
                    name=f'conv_out_layer{conv_count}',
                    kernel_initializer=get_initializer(config.initializer_range)))
                conv_count += 1
    ...
    def call(
        ...
        batch_size = shape_list(hidden_states)[0]
        max_seq_length = shape_list(hidden_states)[1]
        mixed_query_layer = self.query(inputs=hidden_states)
        mixed_key_layer = self.key(inputs=hidden_states)
        mixed_value_layer = self.value(inputs=hidden_states)
        query_layer = self.transpose_for_scores(mixed_query_layer, batch_size)
        ...
        attention_scores_size = query_layer.shape[:-1] + (query_layer.shape[-2],)
        attention_scores = tf.zeros(attention_scores_size)
        ...
        for attention_head in range(self.num_attention_heads):
            if self.attention_types[attention_head] == 'sa':
                if self.sim_types[attention_head] == 'sdp':
                    # Take the dot product between "query" and "key" to get the raw attention scores.
                    attention_scores_list.append(tf.matmul(query_layer[:, attention_head, :, :], 
                        key_layer[:, attention_head, :, :], transpose_b=True))
                elif self.sim_types[attention_head] == 'wma':
                    # Take a weighted multiplicative addition between "query" and "key" vectors.
                    attention_scores_list.append(tf.matmul(tf.matmul(query_layer[:, attention_head, :, :], getattr(self, f'W{wma_count}')), 
                        key_layer[:, attention_head, :, :], transpose_b=True))
                    wma_count += 1
                ...
                attention_scores_list.append(tf.zeros(shape=[s for i, s in enumerate(attention_scores_size) if i != 1]))
            elif self.attention_types[attention_head] == 'c':
                # Attention operation not used in convolution based attention head.
                # Attention scores only used for relative encodings.
                attention_scores_list.append(tf.zeros(shape=[s for i, s in enumerate(attention_scores_size) if i != 1]))

        attention_scores = tf.stack(attention_scores_list, 1)
        dk = tf.cast(self.sqrt_att_head_size, dtype=attention_scores.dtype)
        attention_scores = tf.divide(attention_scores, dk)
        ...
        for attention_head in range(self.num_attention_heads):
            if self.attention_types[attention_head] == 'sa':
                context_layer_list.append(tf.zeros(shape=[batch_size, max_seq_length, self.attention_head_size]))
            elif self.attention_types[attention_head] == 'l':
                if self.sim_types[attention_head] == 'dft':
                    fft_output = fftn(value_layer[:, attention_head, :, :])
                    ...
                    dct_output = tf.signal.dct(value_layer[:, attention_head, :, :])
                    # Add dct to relative position embeddings
                    context_layer_list.append(context_layer[:, attention_head, :, :] + dct_output)
            elif self.attention_types[attention_head] == 'c':
                mixed_key_conv_attn_layer = tf.transpose(getattr(self, f'key_conv_attn_layer{conv_count}')(
                    key_layer[:, attention_head, :, :]), perm=[0, 2, 1])
                mixed_key_conv_attn_layer = tf.transpose(mixed_key_conv_attn_layer, perm=[0, 2, 1])
                # print(f'mixed_key_conv_attn_layer.shape: {mixed_key_conv_attn_layer.shape}')

                conv_attn_layer = tf.math.multiply(mixed_key_conv_attn_layer, query_layer[:, attention_head, :, :])
                conv_kernel_layer = getattr(self, f'conv_kernel_layer{conv_count}')(conv_attn_layer)
                # print(f'conv_kernel_layer.shape: {conv_kernel_layer.shape}')
                conv_kernel_layer = tf.reshape(conv_kernel_layer, [-1, int(self.sim_types[attention_head]), 1])
                conv_kernel_layer = tf.nn.softmax(conv_kernel_layer, axis=1)
                # print(f'conv_kernel_layer.shape after reshape: {conv_kernel_layer.shape}')

                conv_out_layer = getattr(self, f'conv_out_layer{conv_count}')(value_layer[:, attention_head, :, :])
                conv_out_layer = tf.reshape(conv_out_layer, [batch_size, -1, self.attention_head_size])
                conv_out_layer = tf.expand_dims(tf.transpose(conv_out_layer, perm=[0, 2, 1]), axis=-1)
                # print(f'conv_out_layer.shape: {conv_out_layer.shape}')
                conv_out_layer = tf.image.extract_patches(conv_out_layer, sizes=[1, int(self.sim_types[attention_head]), 1, 1], strides=[1, 1, 1, 1], rates=[1, 1, 1, 1], padding='SAME')
                # print(f'conv_out_layer.shape: {conv_out_layer.shape}')
                conv_out_layer = tf.reshape(tf.transpose(conv_out_layer, perm=[0, 2, 1, 3]), [batch_size, -1, self.attention_head_size, int(self.sim_types[attention_head])])
                # print(f'conv_out_layer.shape: {conv_out_layer.shape}')
                conv_out_layer = tf.reshape(conv_out_layer, shape=[-1, self.attention_head_size, int(self.sim_types[attention_head])])
                # print(f'conv_out_layer.shape after reshape: {conv_out_layer.shape}')
                conv_out_layer = tf.matmul(conv_out_layer, conv_kernel_layer)
                conv_out_layer = tf.reshape(conv_out_layer, shape=[-1, self.attention_head_size])

                conv_out = tf.reshape(conv_out_layer, shape=[batch_size, -1, self.attention_head_size])
                conv_count += 1
                
                context_layer_list.append(context_layer[:, attention_head, :, :] + conv_out)

        context_layer = tf.stack(context_layer_list, 1)
...
class TFBertSelfOutputModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        super().__init__(**kwargs)
        ...
        self.dense = tf.keras.layers.Dense(
            units=config.hidden_dim_list[layer_id], kernel_initializer=get_initializer(config.initializer_range), name="dense"
        )
        self.LayerNorm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="LayerNorm")
        self.dropout = tf.keras.layers.Dropout(rate=config.hidden_dropout_prob)
...
class TFBertAttentionModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        super().__init__(**kwargs)

        self.self_attention = TFBertHeteroAttentionModular(config, layer_id, name="self")
        self.dense_output = TFBertSelfOutputModular(config, layer_id, name="output")
...
class TFBertIntermediateModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        super().__init__(**kwargs)

        self.dense = tf.keras.layers.Dense(
            units=config.ff_dim_list[layer_id][0], kernel_initializer=get_initializer(config.initializer_range), name="dense"
        )
        ...
        modules.add(self.dense)
        modules.add(self.intermediate_act_fn)

        for i in range(len(config.ff_dim_list[layer_id])-1):
            modules.add(tf.keras.layers.Dense(units=config.ff_dim_list[layer_id][i+1], kernel_initializer=get_initializer(config.initializer_range), name=f'dense_{i}'))
            modules.add(self.intermediate_act_fn)
...
class TFBertOutputModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, last_layer, **kwargs):
        super().__init__(**kwargs)
        ...
        if self.proj_required:
            self.proj_head = tf.keras.layers.Dense(units=config.hidden_dim_list[layer_id+1], kernel_initializer=get_initializer(config.initializer_range))
...
class TFBertLayerModular(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, last_layer, **kwargs):
        super().__init__(**kwargs)

        self.attention = TFBertAttentionModular(config, layer_id, name="attention")
        self.intermediate = TFBertIntermediateModular(config, layer_id, name="intermediate")
        self.bert_output = TFBertOutputModular(config, layer_id, last_layer, name="output")
...
class TFBertEncoderModular(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        ...
        if config.from_model_dict_hetero:
            for layer_id in range(config.num_hidden_layers):
                self.layer.append(TFBertLayerModular(config, layer_id, layer_id == config.num_hidden_layers - 1, name=f'layer_._{layer_id}'))
        else:
            raise NotImplementedError
...
class TFBertPoolerModular(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)

        self.dense = tf.keras.layers.Dense(
            units=config.hidden_dim_list[-1],
            kernel_initializer=get_initializer(config.initializer_range),
            activation="tanh",
            name="dense",
        )
...
class TFBertPredictionHeadTransformModular(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
...
class TFBertLMPredictionHeadModular(tf.keras.layers.Layer):
    def __init__(self, config, input_embeddings: tf.keras.layers.Layer, **kwargs):
        super().__init__(**kwargs)

        self.vocab_size = config.vocab_size
        self.hidden_size = config.hidden_dim_list[-1]
...
class TFBertMLMHeadModular(tf.keras.layers.Layer):
    def __init__(self, config, input_embeddings: tf.keras.layers.Layer, **kwargs):
        super().__init__(**kwargs)

        self.predictions = TFBertLMPredictionHeadModular(config, input_embeddings, name="predictions")
...
class TFBertNSPHeadModular(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
...
@keras_serializable
class TFBertMainLayerModular(tf.keras.layers.Layer):
    config_class = BertConfig

    def __init__(self, config, add_pooling_layer: bool = True, **kwargs):
        super().__init__(**kwargs)

        self.config = config

        self.embeddings = TFBertEmbeddingsModular(config, name="embeddings")
        self.encoder = TFBertEncoderModular(config, name="encoder")
        self.pooler = TFBertPoolerModular(config, name="pooler") if add_pooling_layer else None
...
class TFBertModelModular(TFBertPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        self.bert = TFBertMainLayerModular(config, name="bert")
...
class TFBertForPreTrainingModular(TFBertPreTrainedModel, TFBertPreTrainingLoss):
    ...
    def __init__(self, config: BertConfig, *inputs, **kwargs):
        ...
        self.bert = TFBertMainLayerModular(config, name="bert")
        self.nsp = TFBertNSPHeadModular(config, name="nsp___cls")
        self.mlm = TFBertMLMHeadModular(config, input_embeddings=self.bert.embeddings, name="mlm___cls")
...
class TFBertForMaskedLMModular(TFBertPreTrainedModel, TFMaskedLanguageModelingLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",
    ...
    def __init__(self, config: BertConfig, *inputs, **kwargs):
        ...
        self.bert = TFBertMainLayerModular(config, add_pooling_layer=False, name="bert")
        self.mlm = TFBertMLMHeadModular(config, input_embeddings=self.bert.embeddings, name="mlm___cls")
...
class TFBertLMHeadModelModular(TFBertPreTrainedModel, TFCausalLanguageModelingLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",
        r"cls.seq_relationship",
...
class TFBertForNextSentencePredictionModular(TFBertPreTrainedModel, TFNextSentencePredictionLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
...
class TFBertForSequenceClassificationModular(TFBertPreTrainedModel, TFSequenceClassificationLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
...
class TFBertForMultipleChoiceModular(TFBertPreTrainedModel, TFMultipleChoiceLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [r"mlm___cls", r"nsp___cls", r"cls.predictions", r"cls.seq_relationship"]
    _keys_to_ignore_on_load_missing = [r"dropout"]
...
class TFBertForTokenClassificationModular(TFBertPreTrainedModel, TFTokenClassificationLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",
        r"mlm___cls",
...
class TFBertForQuestionAnsweringModular(TFBertPreTrainedModel, TFQuestionAnsweringLoss):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",
        r"mlm___cls",
