...
class BertEncoderModular(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayerModular(config,layer_id,layer_id == config.num_hidden_layers - 1) for layer_id in range(config.num_hidden_layers)])
